...
class Predictor(inference.Inference):
    def __init__(self, cfg):
        ...
        self.detector = Detector(detect_type="yolov5_person", prb_thresh=0.5, iou_thresh=0.3)
    ...
    def detect_person(self, rgb_image):
        """进行人体检测"""
    ...
    def predict(self, crops):
        ...
        if len(crops) > 0:
            pred_index, pred_score = self.inference(crops)
    ...
    def start_capture(self, video_file, save_video=None, interval=1, vis=True):
        ...
        while True:
            if count % interval == 0:
                ...
                image = self.show_image(image, dets, pred_score, pred_index, use_rgb=True, vis=vis, delay=5)
...
def get_parser():
    ...
    # 模型文件
    model_file = "data/pretrained/mobilenet_v2_1.0_224_224_CrossEntropyLoss_20230629161618/model/best_model_045_95.5607.pth"
    # 待测试图片目录
    image_dir = "data/test_image"
    # video_file = None  # video_file is None表示进行图片测试
    # video_file = "data/video-test.mp4"  # 视频文件测试
    video_file = "0"  # 摄像头测试
